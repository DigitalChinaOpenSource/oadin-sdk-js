...
class Oadin {
  ...
  async isOadinExisted() {
    ...
    if (existed) {
      await this.isOadinExistedAndUpdate(this.downloadConfig?.version);
      await this.ensureClient();
    }
    return existed;
  }
  ...
  async DownloadEngine(){
    ...
    try {
      ...
      for (const engine of this.downloadConfig.support_engines) {
        ...
        if (res.code !== 200) {
          logAndConsole('error', '下载引擎失败: ' + JSON.stringify(res));
          result = false;
          break;
        }
  ...
  */
  async isOadinExistedAndUpdate(configVersion = null) {
    ...
    try {
      ...
      if (platform === 'win32') {
        const oadinDir = WIN_OADIN_PATH;
        oadinExecutable = path.join(oadinDir, 'oadin.exe');
        ...
        oadinExecutable = MAC_OADIN_PATH; // 确保 MAC_OADIN_PATH 是正确的
        const { stdout } = await new Promise((resolve, reject) => {
          execFile(oadinExecutable, ['version'], { timeout: 5000 }, (error, stdout, stderr) => {
            if (error) {
              logAndConsole('error', `isOadinExistedAndUpdate 执行 'oadin version' 命令失败: ${error.message}, stderr: ${stderr.toString()}`);
